...
use crate::wire::{
    opt_string_size, read_bool, read_char, read_opt_string, read_string, string_size, write_bool, write_char,
    write_opt_string, write_string,
};

/// Placeholder used in place of a secret value anywhere it would otherwise be surfaced.
...
pub enum Request {
    ...
    Status,
    /// Dump the live session property bag, optionally filtered. Secret values are redacted.
    DumpProperties { filter: Option<KeyFilter> },
...
// Manual `Debug` so the secret-bearing `Connect` payload is never printed verbatim.
impl fmt::Debug for Request {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Connect(ps) => f.debug_tuple("Connect").field(&RedactedProperties(ps)).finish(),
            Self::Disconnect => f.write_str("Disconnect"),
            Self::Status => f.write_str("Status"),
            Self::DumpProperties { filter } => f.debug_struct("DumpProperties").field("filter", filter).finish(),
...
/// A [`PropertySet`] whose `Debug` output redacts secret values.
struct RedactedProperties<'a>(&'a PropertySet);
...
pub enum Payload {
    ...
    Status(StatusInfo),
    /// A (redacted) dump of the live property bag.
...
pub struct PropertyDump {
    /// One entry per property, in key order. Secret values are already redacted.
...
pub struct PropertyEntry {
    /// Property key.
    pub key: String,
    /// Property value (already redacted if the key names a secret).
    pub value: PropValue,
    /// Optional short human/LLM-facing description of the property.
...
pub enum PropValue {
    /// Integer value.
    Int(i64),
    /// String value (or the redaction sentinel).
    Str(String),
}

/// A small key filter for [`Request::DumpProperties`]. Matching is case-insensitive.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum KeyFilter {
...
fn write_mouse_button(dst: &mut WriteCursor<'_>, button: MouseButton) -> EncodeResult<()> {
...
fn read_mouse_button(src: &mut ReadCursor<'_>) -> DecodeResult<MouseButton> {
...
fn opt_u16_size(value: Option<u16>) -> usize {
    1 /* presence */ + value.map_or(0, |_| 2)
}

fn write_opt_u16(dst: &mut WriteCursor<'_>, value: Option<u16>) -> EncodeResult<()> {
...
fn read_opt_u16(src: &mut ReadCursor<'_>) -> DecodeResult<Option<u16>> {
    ensure_size!(in: src, size: 1);
    match src.read_u8() {
        0 => Ok(None),
        1 => {
...
impl DecodeOwned for KeyFilter {
...
impl DecodeOwned for PropValue {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_size!(in: src, size: 1);
        match src.read_u8() {
            0 => {
...
impl Encode for PropertyEntry {
    fn encode(&self, dst: &mut WriteCursor<'_>) -> EncodeResult<()> {
        ...
        write_opt_string(dst, self.description.as_deref())
    }
    ...
    fn size(&self) -> usize {
        string_size(&self.key) + self.value.size() + opt_string_size(self.description.as_deref())
    }
}

impl DecodeOwned for PropertyEntry {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        let key = read_string(src)?;
        let value = PropValue::decode_owned(src)?;
        let description = read_opt_string(src)?;
        Ok(Self {
            key,
            value,
            description,
...
impl DecodeOwned for PropertyDump {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_size!(in: src, size: 4);
        let count = src.read_u32();
        let mut entries = Vec::new();
        for _ in 0..count {
            entries.push(PropertyEntry::decode_owned(src)?);
        }
        Ok(Self { entries })
    }
}
...
impl DecodeOwned for StatusInfo {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_size!(in: src, size: 1);
...
impl DecodeOwned for Payload {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_size!(in: src, size: 1);
        match src.read_u8() {
            0 => Ok(Self::Empty),
            1 => Ok(Self::Status(StatusInfo::decode_owned(src)?)),
            2 => Ok(Self::Properties(PropertyDump::decode_owned(src)?)),
            3 => {
                ensure_size!(in: src, size: 4);
                let count = src.read_u32();
...
impl DecodeOwned for Response {
    fn decode_owned(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ensure_size!(in: src, size: 1);
        match src.read_u8() {
            0 => Ok(Self::Ok(Payload::decode_owned(src)?)),
...
impl Encode for Request {
    ...
    fn size(&self) -> usize {
        1 /* tag */
            + match self {
                Self::Connect(properties) => propertyset::size(properties),
                Self::Disconnect | Self::Status | Self::Screenshot => 0,
                Self::DumpProperties { filter } => 1 /* presence */ + filter.as_ref().map_or(0, Encode::size),
                Self::QueryLogs { substring, last } => {
...
mod tests {
    ...
    fn round_trip<T: Encode + DecodeOwned + PartialEq + fmt::Debug>(value: &T) {
        let bytes = ironrdp_core::encode_vec(value).expect("encode");
        assert_eq!(bytes.len(), value.size(), "size() must match bytes written");
        let decoded: T = ironrdp_core::decode_owned(&bytes).expect("decode");
    ...
    fn request_variants_round_trip() {
        ...
        ps.insert("full address", "host:3389");
        ps.insert("ClearTextPassword", "secret");

        round_trip(&Request::Connect(ps));
        round_trip(&Request::Disconnect);
        round_trip(&Request::Status);
        round_trip(&Request::DumpProperties { filter: None });
        round_trip(&Request::DumpProperties {
            filter: Some(KeyFilter::Prefix("ironrdp_".to_owned())),
        });
        round_trip(&Request::QueryLogs {
            substring: Some("connect".to_owned()),
            last: Some(10),
        });
        round_trip(&Request::QueryLogs {
            substring: None,
        ...
        round_trip(&Request::Screenshot);
        round_trip(&Request::MouseMove { x: 100, y: 200 });
        round_trip(&Request::MouseButton {
            button: MouseButton::Right,
            pressed: true,
        });
        round_trip(&Request::Wheel {
            delta: -120,
            horizontal: false,
        });
        round_trip(&Request::KeyScancode {
            scancode: 0xE01D,
            pressed: true,
        });
        round_trip(&Request::KeyUnicode {
            ch: '🚀',
            pressed: false,
        });
    }
    ...
    fn response_variants_round_trip() {
        ...
        round_trip(&Response::error("missing required fields: username, password"));
        round_trip(&Response::Ok(Payload::Status(StatusInfo {
            state: ConnState::Connected,
            destination: Some("host:3389".to_owned()),
            width: Some(1920),
            height: Some(1080),
            message: None,
        })));
        round_trip(&Response::Ok(Payload::Logs(vec![
            "INFO connected".to_owned(),
            "DEBUG frame received".to_owned(),
        ])));
        round_trip(&Response::Ok(Payload::Screenshot {
            width: 800,
            height: 600,
        }));
        round_trip(&Response::Ok(Payload::Properties(PropertyDump {
            entries: vec![
                PropertyEntry {
                    key: "desktopwidth".to_owned(),
                    value: PropValue::Int(1920),
                    description: Some("negotiated remote desktop width in pixels".to_owned()),
                },
                PropertyEntry {
                    key: "ClearTextPassword".to_owned(),
                    value: PropValue::Str(REDACTED.to_owned()),
                    description: None,
                },
            ],
        })));
